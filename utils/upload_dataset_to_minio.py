...
def upload_dataset_to_minio(
    ...
    print(f"[HuggingFace] Downloading dataset '{dataset_name}' (subset: {subset})...")
    if subset:
        ds = load_dataset(dataset_name, subset)
    else:
        ds = load_dataset(dataset_name)

    dataset_key = f"{dataset_name}_{subset}" if subset else dataset_name
    for split in ds.keys():
        records = [row for row in ds[split]]
        jsonl_data = "\n".join(json.dumps(r) for r in records).encode("utf-8")
        data_stream = io.BytesIO(jsonl_data)
        object_name = f"token-classification/{dataset_key}/{split}.jsonl"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="wikiann", help="Hugging Face dataset name")
    parser.add_argument("--subset", type=str, default="en", help="Dataset subset/config")
    parser.add_argument("--bucket", type=str, default="datasets", help="MinIO bucket name")
    args = parser.parse_args()
    upload_dataset_to_minio(dataset_name=args.dataset, subset=args.subset, bucket_name=args.bucket)